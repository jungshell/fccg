...
export default function SchedulePageV2() {
  ...
  // 투표 마감일 계산 (이번주 목요일 17:00 고정) - 메모이제이션 적용
  const getVoteDeadline = useCallback(() => {
    const now = new Date();
    
    // 이번주 목요일 17:00으로 고정
    const currentDay = now.getDay(); // 0: 일요일, 1: 월요일, ..., 6: 토요일
    const daysUntilThursday = currentDay <= 4 ? 4 - currentDay : 4 - currentDay + 7; // 이번주 목요일까지의 일수
    const thisWeekThursday = new Date(now);
    thisWeekThursday.setDate(now.getDate() + daysUntilThursday);
    thisWeekThursday.setHours(17, 0, 0, 0); // 17:00
    
    const deadline = thisWeekThursday;
  ...
  const updateThisWeekScheduleWithGames = useMemo(() => {
    ...
    const updatedData = getScheduleData.thisWeekScheduleData.map(schedule => {
      ...
      // 2. 투표 데이터에서 참석자 수 확인 (지난주 세션)
      if (unifiedVoteData && unifiedVoteData.lastWeekResults && totalCount === 0) {
    ...
    try {
      ...
      if (gamesResponse.ok) {
        ...
        games.forEach((game: any) => {
          ...
          // 확정된 경기만 달력에 표시 (autoGenerated가 false이거나 확정된 정보가 있는 경우)
          if (!game.autoGenerated || (game.time && game.location && game.eventType && 
              game.time !== '미정' && game.location !== '미정' && game.eventType !== '미정')) {
            calendarGameData[dateString] = {
              id: String(game.id ?? dateString),
              confirmed: !game.autoGenerated || (game.time && game.location && game.eventType &&
                game.time !== '미정' && game.location !== '미정' && game.eventType !== '미정'),
              date: gameDate.toISOString(),
              time: game.time || '미정',
              location: game.location || '미정',
              eventType: game.eventType || '미정',
              // 관리자 화면과 동일: (참가회원 + memberNames) + mercenaryCount
              count: (() => {
                const selected = Array.isArray(game.selectedMembers) ? game.selectedMembers : (game.selectedMembers ? JSON.parse(game.selectedMembers) : []);
                const names = Array.isArray(game.memberNames) ? game.memberNames : (game.memberNames ? JSON.parse(game.memberNames) : []);
                const merc = Number(game.mercenaryCount || 0);
                return (selected?.length || 0) + (names?.length || 0) + merc;
              })(),
              // 모달 표시용 원자료 보존
              memberNames: game.memberNames ?? [],
              selectedMembers: game.selectedMembers ?? [],
              mercenaryCount: Number(game.mercenaryCount || 0),
              autoGenerated: game.autoGenerated || false
            };
      ...
      setUnifiedVoteData({
        ...
        lastWeekResults: null // 통합 API에서는 별도로 제공하지 않음
      });
  ...
  const isVoteClosed = useMemo(() => {
    console.log('🔍 투표 마감 상태 확인:', {
      hasActiveSession: !!unifiedVoteData?.activeSession,
      isActive: unifiedVoteData?.activeSession?.isActive,
      remainingHours: voteDeadlineInfo.remainingHours,
      activeSessionData: unifiedVoteData?.activeSession
    });
    
    // 1. 세션이 있고 비활성화된 경우
    if (unifiedVoteData?.activeSession && !unifiedVoteData.activeSession.isActive) {
      console.log('✅ 투표 마감: 세션이 비활성화됨');
      return true;
    }
    // 2. 세션이 없거나 활성화된 경우, 마감일 기준으로 확인
    const deadlineClosed = voteDeadlineInfo.remainingHours <= 0;
    console.log('🔍 마감일 기준 확인:', { deadlineClosed, remainingHours: voteDeadlineInfo.remainingHours });
    return deadlineClosed;
  }, [unifiedVoteData?.activeSession, voteDeadlineInfo.remainingHours]);
                ...
                >
                  ...
                  <Text fontSize={{ base: "xs", md: "sm" }} color="gray.600" fontWeight="medium">
                    투표참여율: {(() => {
                      ...
                      // votes 배열에서 고유한 사용자 수 계산
                      const uniqueVoters = new Set(unifiedVoteData.activeSession.votes?.map(vote => vote.userId) || []).size;
            ...
            {selectedGameData ? (
              <VStack spacing={3} align="stretch">
                ...
                <Flex align="center" gap={2}>
                  ...
                  <Text fontSize="xs" whiteSpace="nowrap">
                    ({(() => {
                      ...
                      const selectedSet = new Set<string>(rawSelectedMembers as string[]);
                      const namesSet = new Set<string>(rawMemberNames as string[]);

                      const memberCount = rawSelectedMembers?.length || 0; // 선택회원은 그대로 회원 수
                ...
                {(() => {
                  ...
                  // 회원 배지: 선택된 회원 그대로 표시
                  rawSelectedMembers.forEach((name: unknown) => {
                    if (typeof name === 'string') participants.push({ name, type: 'member' });
                  });
