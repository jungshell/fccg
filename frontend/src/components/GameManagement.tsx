...
export default function GameManagement({ games, onGamesChange, userList, onGameDataChanged }: GameManagementProps) {
  ...
  const handleDeleteGame = async (id: number | string) => {
    try {
      ...
      const response = await fetch(`${API_ENDPOINTS.BASE_URL}/games/${gameId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const searchLocation = async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`${API_ENDPOINTS.BASE_URL}/search-location?query=${encodeURIComponent(query)}`);
      if (response.ok) {
    ...
    try {
      if (editingGame) {
        ...
        if (editingGame.autoGenerated === true) {
          ...
          const response = await fetch(`${API_ENDPOINTS.BASE_URL}/games`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
        ...
        const requestUrl = isPersistedId
          ? `${API_ENDPOINTS.BASE_URL}/games/${gameId}`
          : `${API_ENDPOINTS.BASE_URL}/games`;
