...
// 토큰 검증 API
router.get('/verify-token', authMiddleware_1.authenticateToken, (req, res) => {
    res.json({
        success: true,
        message: '토큰이 유효합니다.',
        user: req.user
...
// 저장된 투표 결과 조회 API
router.get('/saved-vote-results/:sessionId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
// 토큰 갱신 API
router.post('/refresh-token', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        // 현재 사용자 정보 조회
        const user = await prisma.user.findUnique({
...
const convertKoreanDateToDayCode = (dateStr) => {
...
router.post('/votes', async (req, res) => {
    try {
        ...
        const convertedSelectedDays = selectedDays.map((day) => convertKoreanDateToDayCode(day));
...
router.get('/members', async (req, res) => {
    try {
        ...
        const processedGames = games.map(game => {
            ...
            const uniqueSelected = [...new Set(selectedMembers.filter(n => typeof n === 'string' && actualMemberNames.includes(n)))];
            ...
            const uniqueManual = memberNames.filter(n => {
                ...
                if (!t)
                    return false;
                if (t.startsWith('용병'))
                    return false; // 용병은 별도 카운트
                if (allParticipantNames.includes(t))
                    return false; // 회원과 중복 금지
                return true;
            });
...
// 경기 수정 API
router.put('/games/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
    ...
    try {
        ...
        session.votes.forEach(vote => {
            ...
            selectedDays.forEach((day) => {
                ...
                const dayKey = day;
                if (dayKey && dayVotes[dayKey]) {
                    dayVotes[dayKey].count++;
                    dayVotes[dayKey].participants.push({
                        userId: vote.userId,
                        userName: vote.user.name,
                        votedAt: vote.createdAt
                    });
...
// 경기 삭제 API
router.delete('/games/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
// 투표 세션 마감 API
router.post('/vote-sessions/:id/close', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/cleanup-duplicate-sessions', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
async function reorderSessionNumbers(prisma) {
    try {
...
// 투표 세션 삭제 API
router.delete('/vote-sessions/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const sessionId = parseInt(id);
        if (isNaN(sessionId)) {
...
// 투표 세션 재개 API
router.post('/vote-sessions/:id/resume', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const sessionId = parseInt(id);
        if (isNaN(sessionId)) {
    ...
    try {
        ...
        if (activeSession) {
            const participants = activeSession.votes.map(vote => {
                let selectedDays = vote.selectedDays;
...
router.post('/start-weekly-vote', async (req, res) => {
    try {
        ...
        // 중복 체크 - 정확한 주간(월요일) 비교
        const nextMondayDateOnly = new Date(nextMonday.getFullYear(), nextMonday.getMonth(), nextMonday.getDate());
...
// 회원 수정 API
router.put('/members/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
// 비밀번호 변경 API
router.put('/change-password', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const bcrypt = require('bcrypt');
        ...
        if (activeSession) {
            ...
            activeSession.votes.forEach(vote => {
                ...
                selectedDays.forEach((day) => {
                    ...
                    if (dayKey && dayVotes[dayKey]) {
                        dayVotes[dayKey].count++;
                        dayVotes[dayKey].participants.push(vote.userId);
                    }
                });
            });
            activeSessionData = {
...
// 회원 삭제 API
router.delete('/members/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const memberId = parseInt(req.params.id);
        if (isNaN(memberId)) {
            return res.status(400).json({
      ...
      <div class="content">
        ...
        <div class="participants-section">
          ...
          <div class="participant-list">
            ${data.participants.map(participant => `<span class="participant-tag member">${participant}</span>`).join('')}
            ${data.manualMembers ? data.manualMembers.map(member => `<span class="participant-tag other">${member}</span>`).join('') : ''}
...
// 테스트 알림 발송 API
router.post('/send-test-notification', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
// 실제 파일 업로드 API
router.post('/gallery/upload', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        // 사용자 ID 확인
        const userId = req.user?.userId;
        if (!userId) {
        ...
        req.on('end', async () => {
            try {
                ...
                try {
                    ...
                    const uploadResult = await cloudinary_1.v2.uploader.upload(base64Image, {
                        folder: 'fccg/gallery',
...
// 갤러리 아이템 수정 API
router.put('/gallery/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/gallery/:id/like', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        // 기존 좋아요 확인
...
// 갤러리 아이템 댓글 추가 API
router.post('/gallery/:id/comments', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
// 갤러리 아이템 댓글 수정 API
router.put('/gallery/:id/comments/:commentId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
// 갤러리 아이템 댓글 삭제 API
router.delete('/gallery/:id/comments/:commentId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id, commentId } = req.params;
        const userId = req.user.userId;
        // 댓글 존재 여부 및 권한 확인
...
// 갤러리 아이템 삭제 API
router.delete('/gallery/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.userId;
        // 갤러리 아이템 확인
...
// 게임 관련 API 엔드포인트
// 게임 목록 조회
router.get('/games', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const games = await prisma.game.findMany({
            include: {
                createdBy: {
...
// 게임 생성
router.post('/games', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { date, time, location, gameType, eventType, memberNames, selectedMembers, mercenaryCount, autoGenerated } = req.body;
        const userId = req.user.userId;
...
// 활동 분석 통계 API
router.get('/activity-analysis', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        // 현재 날짜 기준으로 이번 달 계산
        const currentTime = new Date();
            ...
            thisMonthGameParticipations.forEach(game => {
                try {
                    ...
                    const attendedByAttendance = Array.isArray(game.attendances)
                        ? game.attendances.some((a) => {
                            if (a?.userId && a.userId === member.id)
                                return true;
...
scheduleWeeklyVoteSession();
// 데이터 정규화 API (관리자 전용)
router.post('/normalize-data', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: req.user.userId }
        });
