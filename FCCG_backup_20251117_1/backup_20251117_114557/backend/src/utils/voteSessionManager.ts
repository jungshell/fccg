...
export async function createNextWeekSession(): Promise<any> {
  try {
    ...
    console.log(`🎉 새로운 투표 세션 생성 완료: ${newSession.id} (${newSession.weekStartDate.toLocaleDateString('ko-KR')} ~ ${newSession.weekEndDate.toLocaleDateString('ko-KR')})`);
